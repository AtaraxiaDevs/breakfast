...
export default class GameAPI {
  ...
  cargarSprites = function () {
    ...
    this.escena.load.image(
      "s_provisional2",
      "assets/images/s_provisional2.png"
    );
    this.escena.load.spritesheet(
      "sp_velozIddle",
      "assets/images/sprite_veloz_idle.png",
      { frameWidth: 1000, frameHeight: 1000 }
    );
    this.escena.load.spritesheet(
      "sp_velozCorrer",
      "assets/images/sprite_veloz_correr.png",
      { frameWidth: 1000, frameHeight: 1000 }
    );
  };
    ...
    for (let i in dbAnimations) {
      for (let j in dbAnimations[i]) {
        ...
        this.escena.anims.create({
          ...
          key: "veloz_correr",
          frames: this.escena.anims.generateFrameNumbers("sp_velozCorrer", {
            start: 0,
            end: 7,
          }),
          frameRate: 10,
  ...
  dibujarSprite = function (personaje) {
    personaje.setSprite(
      this.escena.physics.add.sprite(200, 400, personaje.sprite)
    );
  ...
  añadirPersonaje = function (tipo) {
    this.personajesRenderizados.push(
      new Personaje(tipo, 200, 400, this.escena)
    );
    this.dibujarSprite(
      this.personajesRenderizados[this.personajesRenderizados.length - 1]
    );
  };
